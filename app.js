...
function init(){
  ...
  let activeScreen = screens.home || screens.host || screens.join || screens.timer || document.querySelector('.screen.active');

  function show(name) {
    const target = screens[name];
    if (!target) return;
    if (activeScreen && activeScreen === screens.timer && name !== 'timer') endGame();
