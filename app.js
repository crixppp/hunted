...
(() => {
  ...
  qs('#btnSpin').addEventListener('click', () => {
    ...
    setTimeout(() => { spinner.style.transition = 'none'; spinning = false; }, 2900);
  });
  ...
  // Reuse previous final roll if present
  ...
  // Adaptive interval: each full minute since Start reduces by 2s, min 20s
  ...
  function scheduleNext(nowMs) {
    currentIntervalSeconds = adaptiveInterval(nowMs);
  ...
  btnStart.addEventListener('click', async () => {
    await ensureAudio();
    // Focused timer-only view
    document.body.classList.add('playing');

    // Remove prestart controls so nothing blocks taps
