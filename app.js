...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function update(id){if(!timerRunning||id!==gameId)return;const now=performance.now();
    ...
    const left=Math.max(0,nextAt-now), displayLeft=Math.max(0,left-DISPLAY_LEAD_MS), sec=Math.ceil(displayLeft/1000);domCountdown.textContent=fmt(sec);
    if(sec<=10)domCountdown.classList.add('red'); else domCountdown.classList.remove('red');
    if(left<=CHIME_LEAD_MS){playChime();schedule(now);} rafId=requestAnimationFrame(()=>update(id));}
  let gameId=null;
